from ..graph.Graph import GraphBase, Edge
...
class GraphAdjacencyList(GraphBase):
    ...
    def addNode(self, elem):
        ...
        """
        new_node = super().addNode(elem)  # create a new node with the correct ID

        self.nodes[new_node.id] = new_node  # add the new node to the dictionary
        self.adj[new_node.id] = List()  # create the adjacency list for the new node
    ...
    def deleteNode(self, node_id):
        ...
        O(m)
        :param node_id: the node ID (integer).
        :return: void.
        """
        try:
            self.nodes[node_id]  # check if nodeId exists
        except KeyError:
